...
const v2purl = $.getval('v2purl'); const v2ptoken = $.getval('v2token');const m_token = $.getval('m_token'); $.KEY_usercfgs = 'chavy_boxjs_userCfgs'
...
!(async () => {
    ...
    for (let i = 0; i < cklist.length; i++) { ckkey = cklist[i]; ckvalue = $.getval(ckkey); console.log(`\n💡开始上传cookie:[${ckkey}]`); await pushcookie(ckkey, ckvalue); }
})().catch((e) => { $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '') }).finally(() => { $.done(); })
async function pushcookie(ckkey, ckvalue) { return new Promise((resolve) => { let url = { url: `${v2purl}/webhook`, body: JSON.stringify({m_token: `${m_token}` , token: `${v2ptoken}`, type: 'store', op: 'put', key: ckkey, value: ckvalue }), headers: { 'Content-Type': 'application/json' }, method: 'get' }; $.post(url, async (err, resp, data) => { try { if (err) { console.log("⛔️API查询请求失败❌ ‼️‼️"); console.log(JSON.stringify(err)); $.logErr(err); } else { $.log(data) } } catch (e) { $.logErr(e, resp); } finally { resolve(); } }); }); }
...
async function apiRevertGlobalBak() {
    ...
    if (backup) {
        const { chavy_boxjs_sysCfgs, chavy_boxjs_sysApps, chavy_boxjs_sessions, chavy_boxjs_userCfgs, chavy_boxjs_cur_sessions, chavy_boxjs_app_subCaches, ...datas } = backup
...
function Env(t, e) {
    ...
    return new class {
        ...
        getdata(t) {
            let e = this.getval(t); if (/^@/.test(t)) { const [, s, i] = /^@(.*?)\.(.*?)$/.exec(t), r = s ? this.getval(s) : ""; if (r) try { const t = JSON.parse(r); e = t ? this.lodash_get(t, i, "") : e } catch (t) { e = "" } }
            return e
        }
        ...
        msg(e = t, s = "", i = "", r) {
            const o = t => {
                if (!t) return t; if ("string" == typeof t) return this.isLoon() ? t : this.isQuanX() ? { "open-url": t } : this.isSurge() ? { url: t } : void 0; if ("object" == typeof t) {
                    if (this.isLoon()) { let e = t.openUrl || t.url || t["open-url"], s = t.mediaUrl || t["media-url"]; return { openUrl: e, mediaUrl: s } }
